 ...
 * be set using `https://<expectedHost>/<URL subpath before "/revocations/">`.
 *
 ...
 * invocation target of `<serviceObjectId>/revocations/:zcapId`.
 *
...
export function authorizeZcapRevocation({
  ...
  async function getExpectedValues({req}) {
    ...
    return {
      ...
      rootInvocationTarget: [
        serviceObjectId,
        `${serviceObjectId}/revocations/${encodeURIComponent(zcapId)}`
      ]
    };
  }
  ...
  return [
    function(req, res, next) {
      ...
      if(!req.url.includes('/revocations/') || !req.params.zcapId) {
        const error = new Error(
    ...
    authorizeZcapInvocationAfterParse({
      documentLoader, getRootController,
      inspectCapabilityChain, onError, suiteFactory
    })
  ];
}
...
function createCheckRevocationMiddleware({
  ...
  return asyncHandler(async function verifyDelegation(req, res, next) {
...
function _parseServiceObjectId({req, expectedHost}) {
  ...
  const idx = req.url.indexOf('/revocations/');
  const path = req.url.substring(0, idx);
