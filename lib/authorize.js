...
export function authorizeZcapInvocation({
  ...
  return asyncHandler(async (req, res, next) => {
    ...
    if(_expectedAction === undefined) {
      _expectedAction = req.method === 'POST' ? 'write' : 'read';
    }

    let expectedRootCapability;
    try {
      expectedRootCapability = await helpers.getExpectedRootCapability({
        req, expectedHost, expectedTarget, expectedAction: _expectedAction,
