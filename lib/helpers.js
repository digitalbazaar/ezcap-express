...
export function createExpectationMiddleware({
  ...
  return asyncHandler(async (req, res, next) => {
    ...
    if(expected.target === undefined) {
      expected.target = `https://${expected.host}${req.url}`;
    }
...
// FIXME: update `wrappedDocumentLoader` to use req.ezcap
// FIXME: rename function to make it clear that it adds the ability to
// look up specific root zcaps to an existing document loader
